...
const login = async (req, res) => {
    try {
        /** Fix - 1
         * Since we are inputting the body using email and password thus 
         * const { username, password } 
         * -> 
         * const { email, password }
         * And,
         * Moreover the user model does not have email and has username column instead so assuming username has user's email,
         * const user = await User.findOne({ username, deleted: false }).populate({path: 'roles'});
         * ->
         * const user = await User.findOne({ username: email, deleted: false }).populate({path: 'roles'});
         * 
         */
        const { email, password } = req.body;
        // Find the user by username
        const user = await User.findOne({ username: email, deleted: false }).populate({
            path: 'roles'
        });
        if (!user) {
        ...
        // Create a JWT token
        /**
         * Fix - 2 Secret Key must be kept in the process.env as other attributes are being kept
         * 'secret_key' -> process.env.AUTH_TOKEN_SECRET
         */
        const token = jwt.sign({ userId: user._id }, process.env.AUTH_TOKEN_SECRET, { expiresIn: '1d' });

        /**
         * Fix - 3 Here the Bearer Token did not have space, it should have that
         */
